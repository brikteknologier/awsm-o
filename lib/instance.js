...
function createEC2Instance(awsmo, opts, callback) {
  ...
  instance.awsKeyName = promise()(null, opts.awsKeyName);
  instance.remoteUser = opts.remoteUser;
  instance.instanceId = awsmo.task(function(callback) {
    awsmo.ec2.then(function(err, ec2) {
      ec2.runInstances({
        ImageId: opts.imageId,
        MinCount: 1,
        MaxCount: 1,
        KeyName: opts.awsKeyName,
  ...
  instance.setName(opts.name || 'AWSM-O instance', callback);
...
function getEC2Instance(awsmo, instanceId, callback) {
  ...
  instance.awsKeyName = promise();
  instance.remoteUser = (awsmo.opts.instanceDefaults || {}).remoteUser;

  resolveAwsKeyName.call(instance, function (err, awsKeyName) {
    instance.awsKeyName(err, awsKeyName);
...
function resolveAwsKeyName(callback) {
  var self = this;
  return this.awsmo.task(function(callback) {
    resolve([self.awsmo.ec2, self.instanceId], function(err, ec2, instanceId) {
      if (err) return callback(err);
      var log = self.log.createSublogger("resolveAwsKeyName(" + instanceId + ")");
      log.info('Asking for instance details...');
      ec2.describeInstances({
        InstanceIds: [ instanceId ]
      }, function(err, results) {
        if (err) return callback(err);
        var keyName = results.Reservations[0].Instances[0].KeyName;
  ...
  return this.awsmo.task(function(callback) {
    resolve([self.awsmo.ec2, self.instanceId], function(err, ec2, instanceId) {
      ec2.stopInstances({ InstanceIds: [ instanceId ] }, function (err, data) {
        if (err) return callback(err);
        self.publicDnsName = promise()(new Error('instance is not running'));
...
Ec2Instance.prototype.start = function(callback) {
  var self = this;
  var task = this.awsmo.task(function(callback) {
...
Ec2Instance.prototype.createAmi = function(baseName, amiDescription, dontStop, callback) {
  var self = this;
  var suffix = 1;
  
  if (typeof dontStop == 'function') {
    callback = dontStop;
    dontStop = false;
  }
