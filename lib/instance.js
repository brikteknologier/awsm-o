...
function createEC2Instance(awsmo, opts, callback) {
  ...
  instance.resolvePublicDnsName(instance.publicDnsName);
  return instance;
}
...
function getState(ec2, instanceId, callback) {
  ec2.describeInstances({ 
    ...
    if (err) return callback(err);
    callback(result.Reservations[0].Instances[0].State.Name);
  });
};
...
Ec2Instance.prototype._catchError = function(err) {
  if (!err) return;
  this.emit('error', err);
};
